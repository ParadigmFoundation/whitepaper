...
\begin{itemize}
  ...
  \item \texttt{Voting} - an generalized contract that manages challenges/votes for the \texttt{ValidatorRegistry} contract, and implements a basic commit-reveal voting scheme. Vote transactions interact directly with the contract (no proxy is used). The contract is sufficiently general to support future protocol voting mechanisms (see \ref{future-work-governance}).
\end{itemize}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item Vote weight is determined by the system balance of a user within the Kosu contract system\footnote{\ All a user's tokens within the contract system count towards vote weight, including any amount locked in either registry.}
